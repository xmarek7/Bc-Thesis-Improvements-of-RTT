...
%Originally, the testing batteries were created to test the random number generators by taking as much data from them as needed. However, common way to test RNG is to take a sample of output, store it into a file and then test the file. The tested file is rewound to the beginning before each individual test (each test is executed over the same sequence), but when testing the generator directly, each test is executed on 'fresh' sequence. 
