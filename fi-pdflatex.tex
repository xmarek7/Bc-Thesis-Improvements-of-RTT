...
\section{Output}
First difference in output between \emph{RTT} and \emph{rtt-py} is the format of the output. \emph{RTT} offers only one report format, which contains all of the reported information (see Subsection \ref{chap:sols-rtt}). The format is human-readable, but requires rather complicated parsing for computer processing.
...
The main difference in output between \emph{RTT} and \emph{rtt-py} are the reported information. Both toolkits report the resulting second-level p-values for Dieharder and NIST STS batteries. However, unlike \emph{RTT}, the \emph{rtt-py} does not report first-level p-values, which could be usefull for deeper assessment of the results.

For the TestU01 batteries, the reported information are the same for both toolkits. However, in \emph{RTT} report, repetitions of the same individual test are grouped together and their p-values are printed together. In \emph{rtt-py}, each repetiton is treated as individual test. The p-values are printed, but the user has to manually group the tests by their names. This is important, because the TestU01 does not perform the two-level test and if the user wishes to apply it, they have to do so on their own.



% MISSING FEATURES
% first-level p-values  
% running Crush family
% check - alphabit and rabbit
% detekce errors / warnings
