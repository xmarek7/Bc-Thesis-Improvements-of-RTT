...
Random number generators are tools for generating bit sequences with random contents. The desired properties of such sequence are \emph{uniformity} (for each bit, the probability of both zero and one are exactly $1/2$), \emph{independence} (none of the bits is influenced by any other bit) and \emph{unpredictability} (it is impossible to predict next bit by obtaining any number of previous bits). \cite[p. 1-1]{nist_special} The random number generators are of different quality in regard to fulfilling the desired properties of generated sequences.

% rng quality, tu01 guide - 2
Each application of random number generators has different requirements for quality of the used generator. The quality is assessed either by mathematical analysis of the generator structure or by taking a sample output of the generator and subjecting it to \emph{randomness test}, which is the subject of this thesis. \cite[p.~2]{tu01_guide}
...
Currently, there are several available programs for randomness testing. Most notable are NIST STS\footnote{National Institute of Standards and Technology Statistical Test Suite} \cite{nist_site}, TestU01 \cite{tu01_site} and Dieharder \cite{dieharder_orig}. Combining all of the three into one program are the Randomness Testing Toolkit \cite{rtt-site} and its evolution, the Randomness Testing Toolkit in Python \cite{rtt-py-site}.
...
The results of this thesis are upgrades to the \emph{rtt-py} (Chapter \ref{improvements}), which allow full replacement of the \emph{RTT} with the \emph{rtt-py} in terms of functionality. The enhancements of the \emph{rtt-py} alongside the Configuration  Calculator also lead to improved usability of this program.
...
\chapter{Programms for randomness testing}
