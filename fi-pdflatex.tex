...
The first reason for such situations is that the distributions of first-level p-values are usually only approximated in the calculation. This causes an error which accumulates with high number of first-level sequences. Second possibl cause is that the distribution is in fact discrete, but a continuous distribution is used ot calculate the p-value.\cite[p. 7]{bad_day}

To find tests with such distributions, the tests were run... TODO

Possible outcomes of previous situations are non-uniform distributions of the first-level p-values. The first example are discrete distributions of first-level p-values with \emph{low} number of possible p-values. The most extreme examples are distributions with only \emph{a few} possible p-values (TODO EXAMPLE). The less extreme tests have lower hundreds of possible first-level p-values. TODO EXAMPLE
